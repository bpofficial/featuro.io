...
import { deleteFeature } from './handler';
...
describe('Delete Feature', () => {
    ...
    const softRemove = jest.fn().mockImplementation(v => Promise.resolve());
    ...
    const getRepository = jest.fn().mockReturnValue({ softRemove, findOne });
    ...
    describe('when user has permissions', () => {
        it('should delete a feature', async () => {
            ...
            const result = await deleteFeature(evt, null, null);
    ...
    describe("when an internal error occurs", () => {
        it('should fail with a 500 Internal server error', async () => {
            ...
            const result = await deleteFeature(event, null, null);
