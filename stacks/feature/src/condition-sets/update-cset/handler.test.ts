...
import { updateFeature } from './handler';
...
describe('Delete Feature', () => {
    ...
    describe('when user has permissions', () => {
        it('should update a feature and not change read-only fields', async () => {
            const context = { ...user }
            const evt = { ...event, requestContext: { authorizer: { context } } };

            const result = await updateFeature(evt, null, null);
            
            expect(result).toBeTruthy();
            if (result) {
                expect(result.statusCode).toBe(200)
                const body = JSON.parse(result.body);
                expect(body.name).toBe('New Feature Name');
                expect(body.active).toBe(true);
                expect(body.key).toBe(feat.key);
            }
        })
    })
    ...
    describe('when the request body is invalid', () => {
        it('should fail with a 400 Bad Request error', async () => {
            const evt = { ...event, body: JSON.stringify({ name: {}}) };
    ...
    describe("when an internal error occurs", () => {
        it('should fail with a 500 Internal server error', async () => {
            ...
            const result = await updateFeature(event, null, null);
